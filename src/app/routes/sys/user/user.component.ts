...
export class SysUserComponent implements OnInit, AfterViewInit {
    ...
    properties: {
      ...
      accountStatus: {
        ...
        ui: {
          ...
          asyncData: () => this.http.get('/sys/dp/enums', { key: 'SysUser.AccountStatus' }, CACHE_ENABLE)
      ...
      buttons: [
        ...
        },
        {
          ...
          children: [
            ...
            {
              text: '操作日志',
              click: record => this.modal.createStatic(SysLogModalComponent, { businessId: record.id }, { size: 'xl' }).subscribe(() => {})
            }
            // {
            //   text: `权限配置(待开发)`,
  ...
  down() {
    ...
    if (app['exportWay'] === 'async') {
      this.http.get('/sys/user/export', this.sf.value, { responseType: 'blob', observe: 'response' }).subscribe(res => {
