...
export class HeaderNotifyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.count = this.settingService.getData('unreadNoticeAmount');
  }
  ...
  private updateNoticeData(notices: NoticeIconList[]): NoticeItem[] {
    ...
    notices.forEach((item) => {
      ...
      if (typeof newItem.datetime === 'string') {
        ...
        newItem.datetime = new Date(newItem.datetime);
      }
      if (newItem.datetime) {
  ...
  loadData(): void {
    ...
    this.http.post('/sys/notice/top8').subscribe((list) => {
      list.forEach((item: any) => {
        ...
        item.datetime = item.createdTime;
      });
